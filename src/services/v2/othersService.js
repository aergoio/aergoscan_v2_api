import {addCachedTokenData, nftRegisteredCache, schedulerDataCache, tokenRegisteredCache} from "../../caches/caches";
        ...
        for (let i = 0; resultsTo < 20; i++) {
            ...
            if (!hasAddress) {
                ...
                merged.set(resultsTo[i].key, {
                    ... ( otherObj.max_blockno < resultsTo[i].max_blockno ? convObj : otherObj),
...
const existedTxTokenList = async (req, res, next) => {
    ...
    try{
        //-- 보유한/했던 토큰 리스트
        const ownerTxTokenList = await req.apiClient.quickSearchTokenTransfersOwnerList(req.query.q);
        ...
        if (ownerTxTokenList.hits && ownerTxTokenList.hits.length) {
            ...
            ownerTxTokenList.hits = await Promise.all(ownerTxTokenList.hits.map(hit => addCachedTokenData(req.apiClient, hit)))
        }
        // console.log("-----"+ JSON.stringify(ownerTxTokenList));
...
// apiRouterV2.route('/tokenTransfers|nftTransfers').get(async (req, res) => {
    ...
    try {
        ...
        const [
            ...
            req.apiClient.aggregateTransfer(req.query.address,{ gte: "now-30d/d", lte: "now" }, "1d")
