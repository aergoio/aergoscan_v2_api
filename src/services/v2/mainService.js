...
const txHistory = async (req, res, next) => {
    ...
    try {
        ...
        const [txPerDay] = await Promise.all([req.apiClient.aggregateBlocks({ gte: 'now-13d/d', lte: 'now' }, '1d')]);
...
const CachedTxHistory = async (req, res, next) => {
    ...
    try {
        const [txPerDay] = await Promise.all([req.apiClient.aggregateBlocks({ gte: 'now-30d/d', lte: 'now' }, '1d')]);
