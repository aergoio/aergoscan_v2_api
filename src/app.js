...
chainRouter.route("/").get((req, res) => {
    return res.json({
        ...
        chains: cfg.AVAILABLE_NETWORKS.map(
            (chainId) => `${cfg.HOST}/${cfg.VERSION}`
        ),
    });
});
...
chainRouter.route("/swapStat").get(async (req, res) => {
    ...
    const isStale =
        cache.swapDataUpdated &&
        new Date() - cache.swapDataUpdated > swapStatCacheDuration;
    if (!cache.swapData || isStale) {
        const url =
            "https://vb42nx03yf.execute-api.us-east-1.amazonaws.com/prod";
        try {
...
chainRouter.param("version", function (req, res, next, version) {
    // ip filter
    const requestIP = req.connection.remoteAddress;
    if (blacklist.indexOf(requestIP) >= 0) {
        // console.log('access attempt : ' + requestIP)
        return res.json({
            msg: "Access Denied",
        });
    }
